...
class CMenu extends CWidget
{
	...
	protected function renderMenuItem($item)
	{
		...
		{
			$label=$this->linkLabelWrapper===null ? strtr($item['linkTemplate'],array('{label}'=>$item['label'])) : '<'.$this->linkLabelWrapper.'>'.strtr($item['linkTemplate'],array('{label}'=>$item['label'])).'</'.$this->linkLabelWrapper.'>';
			return CHtml::link($label,$item['url'],isset($item['linkOptions']) ? $item['linkOptions'] : array());
		}
		else if(isset($item['url']) && !isset($item['linkTemplate']))
		{
