...
Yii::app()->clientScript->registerScript('add-trans-production', "
var no = 0;
$('.button_ok').click(function(){
if(document.getElementById('Goods_goods_name').value == '' || document.getElementById('components_qty').value == '' || document.getElementById('warehouse_id').value == '' || document.getElementById('desc').value == ''){
	...
	$('#box-table-a > tbody:last').append('<tr id=\"trans_'+no+'\"><td><input type=\"checkbox\" name=\"cek_'+no+'\"></td><td>'
	        		...
	        		'</td><td>'
					+document.getElementById('components_qty').value+
	        		'</td><td>'
	        		+document.getElementById('warehouse_id').options[document.getElementById('warehouse_id').selectedIndex].text+
	        		'<input type=\"hidden\" name=\"Production['+no+'][goods_id]\" value=\"'+document.getElementById('goods_id').value+'\">'+
	        		'<input type=\"hidden\" name=\"Production['+no+'][components_qty]\" value=\"'+document.getElementById('components_qty').value+'\">'+
	        		'<input type=\"hidden\" name=\"Production['+no+'][warehouse_id]\" value=\"'+document.getElementById('warehouse_id').value+'\">'+
	        		'<input type=\"hidden\" name=\"Production['+no+'][decsription]\" value=\"'+document.getElementById('desc').value+'\">'+
...
<!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="dashboard">Production</h1>
    </div>
    <!--RIGHT TEXT/CALENDAR-->
    ...
    <!-- FIRST SORTABLE COLUMN START -->
      ...
      <!--THIS IS A PORTLET-->
        <div class="portlet">
		<div class="portlet-header">Form Production</div>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                ...
                <th width="25%" scope="col">Goods Name</th>
                <th width="25%" scope="col">Components Qty</th>
                <th width="25%" scope="col">Warehouse</th>
