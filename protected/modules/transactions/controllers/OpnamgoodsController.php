...
class OpnamGoodsController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['OpnamGoods']))
		{
			$err = 0;
			foreach($_POST['OpnamGoods'] as $postrans){
				$model=new OpnamGoods;
				//id_trans, qty, date_time, decsription, balance_history, component_id, user_id
				$model->goods_id=$postrans['goods_id'];
				$model->stock_goods=$postrans['stock_goods'];
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['OpnamGoods']))
		{
			$model->attributes=$_POST['OpnamGoods'];
			if($model->save())
	...
	{
		$model=new OpnamGoods('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['OpnamGoods']))
			$model->attributes=$_GET['OpnamGoods'];
	...
	{
		$model=OpnamGoods::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='opnam-goods-form')
		{
