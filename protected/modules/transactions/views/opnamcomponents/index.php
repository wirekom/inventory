...
if(document.getElementById('Components_component_name').value == '' || document.getElementById('warehouse_id').value == '' || document.getElementById('stock_component').value == '' || document.getElementById('office').value == '' || document.getElementById('desc').value == ''){
	...
	$.ajax({
		...
		dataType : 'json',
		cache : false,
		url : '".Yii::app()->request->baseUrl."/index.php/transactions/opnamcomponents/addtrans',
		data : {
		...
		success : function(data){
			$('#box-table-a > tbody:last').append('<tr id=\"trans_'+no+'\"><td><input type=\"checkbox\" name=\"cek_'+no+'\"></td><td>'
	        		...
	        		'</td><td>'
					+document.getElementById('stock_component').value+
	        		'</td><td>'
	        		+document.getElementById('office').value+
	        		'<input type=\"hidden\" name=\"OpnamComponents['+no+'][component_id]\" value=\"'+data.component_id+'\">'+
	        		'<input type=\"hidden\" name=\"OpnamComponents['+no+'][warehouse_id]\" value=\"'+document.getElementById('warehouse_id').value+'\">'+
	        		'<input type=\"hidden\" name=\"OpnamComponents['+no+'][stock_component]\" value=\"'+document.getElementById('stock_component').value+'\">'+
	        		'<input type=\"hidden\" name=\"OpnamComponents['+no+'][office]\" value=\"'+document.getElementById('office').value+'\">'+
	        		'<input type=\"hidden\" name=\"OpnamComponents['+no+'][decsription]\" value=\"'+document.getElementById('desc').value+'\">'+
...
<!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="dashboard">Opnam Components</h1>
    </div>
    <!--RIGHT TEXT/CALENDAR-->
          ...
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                ...
                <th width="15%" scope="col">Stock Components</th>
